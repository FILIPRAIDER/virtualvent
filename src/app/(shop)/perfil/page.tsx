...
export default function ProfilePage() {
  ...
  return (
    ...
    {/* <pre>{JSON.stringify(session?.user, null, 2)}</pre>
      ...
      <section className="flex flex-col px-40">
        ...
        <img className="w-12 h-12 rounded-full object-cover" src={`https://ui-avatars.com/api/?name=${session?.user?.name}&background=0D8ABC&color=fff`} alt="Avatar" />
       
    </div>
    <div className="ml-1">
        <h2 className="font-semibold text-gray-800 text-lg">{session?.user?.name}</h2>
        <p className="text-gray-600 text-sm font-regular">Montería, Córdoba</p>
    </div>
  ...
  <form className="space-y-6">
      ...
      <div className="flex gap-2 items-end">
        <div>
          ...
          <div className="flex gap-2 border  rounded-md border-gray-300 px-2 py-2 text-sm shadow-sm ">
            ...
            <input type="text" className="flex-1" placeholder="Numero de documento" />
          </div>
        </div>
      </div>
    </div>
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
      ...
      <div>
        ...
        <input type="date" className="mt-1 w-full rounded-md border border-gray-300 px-3 py-2 text-sm shadow-sm" />
      </div>
      <div>
  ...
  <section>
  <div className="mt-4">
  <h2 className="text-xl font-semibold text-gray-900 mb-1">Cambiar Contraseña</h2>
  <hr className="mb-6 border-gray-300" />

  <form className="space-y-6">
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
      <div>
        <label className="block text-sm font-medium text-gray-700">Contraseña antigua</label>
        <input type="text" className="mt-1 w-full rounded-md border border-gray-300 px-3 py-2 text-sm shadow-sm" />
      </div>
      <div>
